...
public class FunctionalEventsByTagPublisher extends AbstractActorPublisher<EventEnvelope> {

	private final FunctionalDao dao;
	...
	public FunctionalEventsByTagPublisher(FunctionalDao dao, String tag, Timestamp offset, FiniteDuration refreshInterval) {
		this.dao = dao;
		this.tag = tag;
	...
	public static Props props(FunctionalDao dao, String tag, Timestamp offset, FiniteDuration refreshInterval) {
		return Props.create(FunctionalEventsByTagPublisher.class, dao, tag, offset, refreshInterval);
	}
	...
	private void query() {
		if (buf.isEmpty()) {
			...
			List<Messages> messages = dao.getEventsByTag(tag, currentOffset, LIMIT);
